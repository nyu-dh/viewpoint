	...
	})()(
		{
			1: [
				function (require, module, exports) {
					 ...
					 * @constructor
					 */
					var WaveformDataArrayBufferAdapter = (module.exports =
 ...
 */
					WaveformDataArrayBufferAdapter.fromResponseData = function fromArrayBufferResponseData(
						response_data
					) {
			...
			3: [
				function (require, module, exports) {
					 ...
					 * @constructor
					 */
					var WaveformDataObjectAdapter = (module.exports = function WaveformDataObjectAdapter(
						response_data
					) {
						this.data = response_data;
					});
					...
					WaveformDataObjectAdapter.isCompatible = function isCompatible(data) {
						return (
							(data && typeof data === 'object' && 'sample_rate' in data) ||
 ...
 */
					WaveformDataObjectAdapter.fromResponseData = function fromJSONResponseData(
						response_data
					) {
				...
				function (require, module, exports) {
					...
					WaveformData.create = function createFromResponseData(data) {
						...
						if (
							data &&
							typeof data === 'object' &&
							('responseText' in data || 'response' in data)
						) {
					...
					WaveformData.prototype = {
						...
						offset: function (start, end) {
							...
							if (end < start) {
								throw new RangeError(
									'End point must not be before the start point [' +
										Number(end) +
										' < ' +
										Number(start) +
										']'
							...
							if (start < 0) {
								throw new RangeError(
									'Start point must be non-negative [' + Number(start) + ' < 0]'
								);
							}

							if (start >= data_length) {
								throw new RangeError(
									'Start point must be within range [' + Number(start) + ' >= ' + data_length + ']'
								);
							}
						...
						resample: function (options) {
							...
							options.input_index =
								typeof options.input_index === 'number' ? options.input_index : null;
							options.output_index =
								typeof options.output_index === 'number' ? options.output_index : null;
							...
							var samples_per_pixel =
								options.scale ||
								Math.floor((this.duration * this.adapter.sample_rate) / options.width); //scale we want to reach
			...
			6: [
				function (require, module, exports) {
					 ...
					 * @constructor
					 */
					var WaveformDataSegment = (module.exports = function WaveformDataSegment(
						context,
						start,
						end
					) {
					...
					WaveformDataSegment.prototype = {
						...
						get visible() {
							return (
								this.context.in_offset(this.start) ||
						...
						get min() {
							return this.visible
								? this.context.offsetValues(this.offset_start, this.offset_length, 0)
								: [];
						},
						/**
						...
						get max() {
							return this.visible
								? this.context.offsetValues(this.offset_start, this.offset_length, 1)
								: [];
						}
					};
				},
