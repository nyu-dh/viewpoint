							...
							if (typeof value === 'string') {
								...
								for (var i = 0, total = value.length; i < total; i++) {
									...
									mediaFiles.push({
										src: src,
										type:
											(type === '' || type === null || type === undefined) && src
			...
			function (_dereq_, module, exports) {
				...
				Object.assign(_player2.default.prototype, {
					...
					buildprogress: function buildprogress(player, controls, layers, media) {
						...
						controls.querySelector('.' + t.options.classPrefix + 'time-buffering').style.display =
							'none';
						...
						t.slider.addEventListener('keydown', function (e) {
							...
							if (t.options.keyActions.length) {
								...
								seekTime =
									seekTime < 0 ? 0 : seekTime >= duration ? duration : Math.floor(seekTime);
						...
						if (t.media.currentTime !== undefined && t.media.duration) {
							...
							if (t.total && t.handle) {
								var newWidth = Math.round(
										(parseFloat(getComputedStyle(t.total).width) * nTime) / t.media.duration
									),
				...
				Object.assign(_player2.default.prototype, {
					...
					updateCurrent: function updateCurrent() {
						...
						if (t.controls.querySelector('.' + t.options.classPrefix + 'currenttime')) {
							t.controls.querySelector('.' + t.options.classPrefix + 'currenttime').innerText = (0,
					...
					updateDuration: function updateDuration() {
						...
						if (
							t.controls.querySelector('.' + t.options.classPrefix + 'duration') &&
							duration > 0
						) {
				...
				Object.assign(_player2.default.prototype, {
					...
					buildtracks: function buildtracks(player, controls, layers, media) {
						...
						if (!player.options.alwaysShowControls) {
							...
							(0, _dom.addClass)(
								player.container.querySelector('.' + t.options.classPrefix + 'captions-position'),
								t.options.classPrefix + 'captions-position-hover'
							);
						}
					...
					adjustLanguageBox: function adjustLanguageBox() {
						...
						t.captionsButton.querySelector(
							...
							parseFloat(
								t.captionsButton.querySelector(
									'.' + t.options.classPrefix + 'captions-selector-list'
								).offsetHeight
							) + 'px';
					},
			...
			function (_dereq_, module, exports) {
				...
				Object.assign(_player2.default.prototype, {
					...
					buildvolume: function buildvolume(player, controls, layers, media) {
						...
						var volumeSlider =
								mode === 'vertical'
									? t.container.querySelector('.' + t.options.classPrefix + 'volume-slider')
									: t.container.querySelector(
											'.' + t.options.classPrefix + 'horizontal-volume-slider'
									  ),
							volumeTotal =
								mode === 'vertical'
									? t.container.querySelector('.' + t.options.classPrefix + 'volume-total')
									: t.container.querySelector(
											'.' + t.options.classPrefix + 'horizontal-volume-total'
									  ),
							volumeCurrent =
								mode === 'vertical'
									? t.container.querySelector('.' + t.options.classPrefix + 'volume-current')
									: t.container.querySelector(
											'.' + t.options.classPrefix + 'horizontal-volume-current'
									  ),
							volumeHandle =
								mode === 'vertical'
									? t.container.querySelector('.' + t.options.classPrefix + 'volume-handle')
									: t.container.querySelector(
					...
					keyActions: [
						...
						},
						{
							...
							action: function action(player, media) {
								if (
									player.container
										.querySelector('.' + config.classPrefix + 'volume-button>button')
										.matches(':focus') ||
									player.container
					...
					function MediaElementPlayer(node, o) {
						...
						if (
							...
							t.container.addEventListener('focus', function (e) {
								if (!t.controlsAreVisible && !t.hasFocus && t.controlsEnabled) {
									...
									if (!_constants.HAS_MS_NATIVE_FULLSCREEN) {
										...
										var btnSelector = (0, _general.isNodeAfter)(e.relatedTarget, t.container)
												? '.' +
												  t.options.classPrefix +
												  'controls .' +
												  t.options.classPrefix +
							...
							if (!t.options.features.length) {
								...
								t.container.querySelector('.' + t.options.classPrefix + 'controls').style.display =
									'none';
							}
							...
							if (
								...
								!dom.hasClass(t.container.parentNode, t.options.classPrefix + 'fill-container')
							) {
								// outer container
					...
					_createClass(MediaElementPlayer, [
								...
								if (
									...
									(!t.controlsAreVisible ||
										...
										(t.media.paused &&
											...
											((!t.options.hideVideoControlsOnLoad && t.media.currentTime <= 0) ||
												(!t.options.hideVideoControlsOnPause && t.media.currentTime > 0))) ||
									...
									var _ret5 = (function () {
										...
										t.globalBind('keydown', function (e) {
											if (e.target.matches('.' + t.options.classPrefix + 'container')) {
												...
												dom.removeClass(
													event.target.closest('.' + t.options.classPrefix + 'container'),
													t.options.classPrefix + 'container-keyboard-inactive'
												);
											}
										});
						...
						},
						{
							...
							value: function hasFluidMode() {
								...
								return (
									...
									(t.node.style.maxWidth !== 'none' && t.node.style.maxWidth !== t.width) ||
						...
						},
						{
							...
							value: function setDimensions(width, height) {
								...
								width =
									(0, _general.isString)(width) && width.includes('%')
										? width
										: parseFloat(width) + 'px';
								height =
									(0, _general.isString)(height) && height.includes('%')
										? height
						...
						},
						{
							...
							value: function setControlsSize() {
								...
								var railStyles = getComputedStyle(t.rail),
									...
									totalMargin =
										parseFloat(totalStyles.marginLeft) + parseFloat(totalStyles.marginRight) || 0;
								...
								t.rail.style.width =
									(siblingsWidth > controlsWidth ? 0 : controlsWidth - siblingsWidth) + 'px';
							...
							value: function buildoverlays(player, controls, layers, media) {
								...
								if (
									...
									((t.media.rendererName.match(/(youtube|facebook)/) &&
										!(player.media.originalNode.getAttribute('poster') || player.options.poster)) ||
		...
		25: [
			function (_dereq_, module, exports) {
				...
				function isNodeAfter(sourceNode, targetNode) {
					return !!(
						(sourceNode && targetNode && sourceNode.compareDocumentPosition(targetNode) & 2) // 2 : Node.DOCUMENT_POSITION_PRECEDING
					);
				}
		...
		26: [
			function (_dereq_, module, exports) {
				...
				function getTypeFromFile(url) {
					...
					if (normalizedExt) {
						if (
							['mp4', 'm4v', 'ogg', 'ogv', 'webm', 'flv', 'mpeg', 'mov'].includes(normalizedExt)
						) {
