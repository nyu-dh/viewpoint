...
(function (IIIFComponents) {
	...
	var MetadataComponent = /** @class */ (function (_super) {
		...
		MetadataComponent.prototype._getManifestGroup = function () {
			return this._metadataGroups
				.en()
				.where(function (x) {
					return x.resource.isManifest();
				})
				.first();
		};
		...
		MetadataComponent.prototype.set = function (data) {
			...
			if (this._data.canvasDisplayOrder) {
				...
				canvasGroups.forEach(function (canvasGroup, index) {
					canvasGroup.items = _this._sortItems(
						canvasGroup.items,
						_this._readCSV(_this._data.canvasDisplayOrder)
					);
				});
			}
			...
			if (this._data.canvasExclude) {
				var canvasGroups = this._getCanvasGroups();
				canvasGroups.forEach(function (canvasGroup, index) {
					canvasGroup.items = _this._exclude(
						canvasGroup.items,
						_this._readCSV(_this._data.canvasExclude)
					);
				});
			}
		...
		MetadataComponent.prototype._sortItems = function (items, displayOrder) {
			...
			displayOrder.forEach(function (item, index) {
				var match = unsorted
					.en()
					.where(function (x) {
						return _this._normalise(x.getLabel()) === item;
					})
					.first();
				if (match) {
					sorted.push(match);
		...
		MetadataComponent.prototype._sortGroups = function (groups, metadataGroupOrder) {
			...
			metadataGroupOrder.forEach(function (group, index) {
				var match = unsorted
					.en()
					.where(function (x) {
						return x.resource.constructor.name.toLowerCase() == group;
					})
					.first();
				if (match) {
					sorted.push(match);
...
(function (IIIFComponents) {
	...
	(function (MetadataComponent) {
		...
		(MetadataComponent =
			IIIFComponents.MetadataComponent || (IIIFComponents.MetadataComponent = {}))
	);
})(IIIFComponents || (IIIFComponents = {}));
(function (g) {
	if (!g.IIIFComponents) {
