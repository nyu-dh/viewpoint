...
(function (IIIFComponents) {
	var AVVolumeControl = /** @class */ (function (_super) {
		...
		AVVolumeControl.prototype._init = function () {
			...
			this._$volumeMute = $(
				'\n                                <button class="btn volume-mute" title="' +
	...
	var CanvasInstance = /** @class */ (function (_super) {
		...
		CanvasInstance.prototype.init = function () {
							...
							r.forEach(function (range) {
								...
								if (clonedRange.canvases && clonedRange.canvases.length) {
									for (var i = 0; i < clonedRange.canvases.length; i++) {
										clonedRange.canvases[i] = AVComponentUtils.retargetTemporalComponent(
											_this._data.canvas.canvases,
			...
			this._$rangeTimelineContainer.on('mousemove', function (e) {
				if (_this._data.range) {
					...
					_this._updateHoverPreview(
						e,
						_this._$rangeTimelineContainer,
			...
			for (var i = 0; i < items.length; i++) {
                ...
                */
				...
				var percentageTop = this._convertToPercentage(positionTop, this._canvasHeight),
		...
		CanvasInstance.prototype._renderSyncIndicator = function (mediaElementData) {
			...
			var widthPercent = this._convertToPercentage(
	...
	var AVComponentUtils = /** @class */ (function () {
		...
		AVComponentUtils.diff = function (a, b) {
			return Array.from(
				new Set(AVComponentUtils._compare(a, b).concat(AVComponentUtils._compare(b, a)))
			);
		};
	...
	var AVComponent = /** @class */ (function (_super) {
		...
		AVComponent.prototype._initCanvas = function (canvas) {
			...
			canvasInstance.on(
				AVComponent.Events.MEDIA_READY,
				function () {
					_this._readyMedia++;
				},
				false
			...
			canvasInstance.on(
				AVComponent.Events.WAVEFORM_READY,
				function () {
			...
			canvasInstance.on(
				AVComponent.Events.RANGE_CHANGED,
				function (rangeId) {
			...
			if (this._data.virtualCanvasEnabled) {
				for (var i = 0; i < this.canvasInstances.length; i++) {
					...
					if (currentCanvasId) {
						...
						if (
							((canvasInstance.isVirtual() || this.canvasInstances.length === 1) &&
