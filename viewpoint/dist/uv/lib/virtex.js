											...
											function (materials) {
												...
												objLoader.load(
													...
													function (obj) {
														...
														bufferGeometry.computeBoundingBox();
														var sizeX =
															bufferGeometry.boundingBox.max.x - bufferGeometry.boundingBox.min.x;
														var sizeY =
															bufferGeometry.boundingBox.max.y - bufferGeometry.boundingBox.min.y;
														var sizeZ =
															bufferGeometry.boundingBox.max.z - bufferGeometry.boundingBox.min.z;
														var diagonalSize = Math.sqrt(
							...
							var Viewport = /** @class */ (function () {
								...
								Viewport.prototype.createLights = function () {
									...
									var light1 = new THREE.DirectionalLight(
										this.options.data.directionalLight1Color,
										this.options.data.directionalLight1Intensity
									);
									light1.position.set(1, 1, 1);
									this._lightGroup.add(light1);
									var light2 = new THREE.DirectionalLight(
										this.options.data.directionalLight2Color,
										this.options.data.directionalLight2Intensity
									);
									light2.position.set(-1, -1, -1);
									this._lightGroup.add(light2);
									var ambientLight = new THREE.AmbientLight(
										this.options.data.ambientLightColor,
										this.options.data.ambientLightIntensity
									);
								...
								Viewport.prototype.createCamera = function (camera) {
									if (camera) {
										...
										this.camera = new THREE.PerspectiveCamera(
											...
											this.options.data.near,
											this.options.data.far
										);
								...
								Viewport.prototype._createDOMHandlers = function () {
									...
									window.addEventListener(
										...
										this._onPointerRestricted.bind(this),
										false
									);
									window.addEventListener(
										'vrdisplaypointerunrestricted',
										this._onPointerUnrestricted.bind(this),
										false
									);
											...
											function (obj) {
												switch (_this.options.data.type.toString()) {
													...
													case Virtex.FileType.THREEJS.toString():
														...
														break;
													case Virtex.FileType.OBJ.toString():
														Virtex.ObjFileTypeHandler.setup(_this, objectPath, obj).then(function (
															obj
														) {
															_this._loaded(obj);
															resolve();
        ...
        */
								...
								Viewport.prototype._onMouseMove = function (event) {
									...
									if (this._isMouseDown) {
										this._targetRotation.y =
											this._targetRotationOnMouseDown.y +
											(this._mousePos.y - this._mousePosOnMouseDown.y) * 0.02;
										this._targetRotation.x =
											this._targetRotationOnMouseDown.x +
											(this._mousePos.x - this._mousePosOnMouseDown.x) * 0.02;
									}
								};
								...
								Viewport.prototype._onTouchMove = function (event) {
									...
									switch (touches.length) {
										case 1: // one-fingered touch: rotate
											...
											this._targetRotation.x =
												this._targetRotationOnMouseDown.x +
												(this._mousePos.x - this._mousePosOnMouseDown.x) * 0.05;
											this._mousePos.y = touches[0].pageY - this._viewportCenter.y;
											this._targetRotation.y =
												this._targetRotationOnMouseDown.y +
												(this._mousePos.y - this._mousePosOnMouseDown.y) * 0.05;
											break;
								...
								Viewport.prototype.resize = function () {
									if (this._element && this._viewport) {
										...
										this.camera.updateProjectionMatrix();
										this.renderer.setSize(this._viewport.offsetWidth, this._viewport.offsetHeight);
										this._loading.style.left =
											String(this._viewportCenter.x - this._loading.offsetWidth / 2) + 'px';
										this._loading.style.top =
											String(this._viewportCenter.y - this._loading.offsetHeight / 2) + 'px';
									} else if (this._oldie) {
										this._oldie.style.left =
											String(this._element.offsetWidth / 2 - this._oldie.offsetWidth / 2) + 'px';
										this._oldie.style.top =
											String(this._element.offsetHeight / 2 - this._oldie.offsetHeight / 2) + 'px';
									}
								};
								return Viewport;
							})();
