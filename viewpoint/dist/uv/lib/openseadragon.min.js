...
!(function ($) {
	...
	var isIOSDevice = function () {
		...
		return 'string' != typeof a
			? !1
			: -1 !== a.indexOf('iPhone') || -1 !== a.indexOf('iPad') || -1 !== a.indexOf('iPod');
	};
	$.extend($, {
		DEFAULT_SETTINGS: {
			xmlPath: null,
		...
		getElementOffset: function (a) {
			...
			c = d == d.window ? d : 9 === d.nodeType ? d.defaultView || d.parentWindow : !1;
			return new $.Point(
				e.left + (c.pageXOffset || b.scrollLeft) - (b.clientLeft || 0),
				e.top + (c.pageYOffset || b.scrollTop) - (b.clientTop || 0)
			);
		},
		...
		makeCenteredNode: function (a) {
			...
			$.extend(b[0].style, { display: 'table', height: '100%', width: '100%' });
			$.extend(b[1].style, { display: 'table-row' });
			$.extend(b[2].style, { display: 'table-cell', verticalAlign: 'middle', textAlign: 'center' });
			b[0].appendChild(b[1]);
			b[1].appendChild(b[2]);
			b[2].appendChild(a);
			return b[0];
		...
		setElementOpacity: function (a, b, c) {
			...
			if ($.Browser.opacity) a.style.opacity = 1 > b ? b : '';
			else if (1 > b) {
				d = Math.round(100 * b);
				e = 'alpha(opacity=' + d + ')';
				a.style.filter = e;
		...
		indexOf: function (a, b, c) {
			Array.prototype.indexOf
				...
				: (this.indexOf = function (a, b, c) {
						...
						if (0 === e || f >= e) return -1;
						0 > f && (f = e - Math.abs(f));
						for (d = f; e > d; d++) if (a[d] === b) return d;
						return -1;
				  });
		...
		makeAjaxRequest: function (a, b, c) {
			...
			var e = $.getUrlProtocol(a);
			var f = $.createAjaxRequest('file:' === e);
			if (!$.isFunction(b)) throw new Error('makeAjaxRequest requires a success callback');
			f.onreadystatechange = function () {
				if (4 == f.readyState) {
					f.onreadystatechange = function () {};
					if (200 === f.status || (0 === f.status && 'http:' !== e && 'https:' !== e)) b(f);
					else {
						$.console.log('AJAX request returned %d: %s', f.status, a);
						$.isFunction(c) && c(f);
					}
				}
			};
			...
			try {
				...
				var h = g.message;
				var i = $.Browser.vendor == $.BROWSERS.IE && $.Browser.version < 10;
				i &&
					'undefined' != typeof g.number &&
					-2147024891 == g.number &&
					(h += '\nSee http://msdn.microsoft.com/en-us/library/ms537505(v=vs.85).aspx#xdomain');
				$.console.log('%s while making AJAX request: %s', g.name, h);
				f.onreadystatechange = function () {};
				if (window.XDomainRequest) {
					var j = new XDomainRequest();
					if (j) {
						j.onload = function (a) {
							$.isFunction(b) && b({ responseText: j.responseText, status: 200, statusText: 'OK' });
						};
			...
			case 'Microsoft Internet Explorer':
				if (window.attachEvent && window.ActiveXObject) {
					...
					$.Browser.version = parseFloat(
						c.substring(c.indexOf('MSIE') + 5, c.indexOf(';', c.indexOf('MSIE')))
					);
				}
				break;
			case 'Netscape':
...
!(function (a) {
	...
	a.EventSource.prototype = {
		...
		addHandler: function (b, c, d) {
			var e = this.events[b];
			e || (this.events[b] = e = []);
			c && a.isFunction(c) && (e[e.length] = { handler: c, userData: d || null });
		},
...
!(function (a) {
		...
		for (c = 0; e > c; c++)
			if (d.activePointersLists[c].captureCount > 0) {
				...
				a.removeEvent(
					...
					a.MouseTracker.unprefixedPointerEvents ? 'pointermove' : 'MSPointerMove',
					d.pointermovecaptured,
					!0
				);
				a.removeEvent(
					a.MouseTracker.captureElement,
					a.MouseTracker.unprefixedPointerEvents ? 'pointerup' : 'MSPointerUp',
					d.pointerupcaptured,
					!0
				);
	...
	function f(b, c, d) {
		var f,
			g = b.getActivePointersListByType(c);
		g.captureCount += d || 1;
		if (1 === g.captureCount)
	...
	function u(b, c) {
		...
		'mousewheel' == a.MouseTracker.wheelEventName
			? (d.deltaY = (-1 / a.DEFAULT_SETTINGS.pixelsPerWheelLine) * c.wheelDelta)
			: (d.deltaY = c.detail);
		v(b, d, c);
	}
	function v(b, c, d) {
	...
	function T(a, b) {
		var c = (b.changedTouches.length, a.getActivePointersListByType('touch'));
		L(a, b, c);
	}
	function U(a, b) {
		b.stopPropagation();
	...
	function ga(b, c, d) {
		var e,
			...
			g,
			h,
			i = b.getActivePointersListByType(d[0].type),
			j = d.length;
		for (e = 0; j > e; e++) {
			f = d[e];
			g = i.getById(f.id);
	...
	function ha(b, c, d) {
		var e,
			...
			g,
			h,
			i = (pa[b.hash], b.getActivePointersListByType(d[0].type)),
			j = d.length;
		for (e = 0; j > e; e++) {
			f = d[e];
			g = i.getById(f.id);
	...
	function ia(b, c, d, e) {
		...
		for (g = 0; n > g; g++) {
			...
			(b.dragHandler || b.dragEndHandler || b.pinchHandler) &&
				a.MouseTracker.gesturePointVelocityTracker.addPoint(b, h);
			if (1 === m.contacts) {
				if (b.pressHandler) {
					f = b.pressHandler({
						eventSource: b,
				...
				j.lastPinchDist = j.currentPinchDist = j.pinchGPoints[0].currentPos.distanceTo(
		...
		for (i = 0; q > i; i++) {
			...
			if (m) {
				...
				if (s) {
					p.contacts--;
					(b.dragHandler || b.dragEndHandler || b.pinchHandler) &&
						a.MouseTracker.gesturePointVelocityTracker.removePoint(b, m);
					if (0 === p.contacts) {
						if (b.releaseHandler) {
							f = b.releaseHandler({
								eventSource: b,
						...
						if ((b.clickHandler || b.dblClickHandler) && m.insideElement) {
							n =
								h - m.contactTime <= b.clickTimeThreshold &&
	...
	a.MouseTracker = function (b) {
		...
		var c = arguments;
		a.isPlainObject(b) ||
			(b = { element: c[0], clickTimeThreshold: c[1], clickDistThreshold: c[2] });
		...
		this.dblClickTimeThreshold =
			b.dblClickTimeThreshold || a.DEFAULT_SETTINGS.dblClickTimeThreshold;
		this.dblClickDistThreshold =
			b.dblClickDistThreshold || a.DEFAULT_SETTINGS.dblClickDistThreshold;
	...
	a.MouseTracker.gesturePointVelocityTracker = (function () {
		...
		var f = function () {
			...
			for (c = 0; j > c; c++) {
				...
				i = (1e3 * h) / (g + 1);
				f.speed = 0.75 * i + 0.25 * f.speed;
			}
		};
		var g = function (g, h) {
			var i = e(g, h);
			b.push({ guid: i, gPoint: h, lastPos: h.currentPos });
			if (1 === b.length) {
				d = a.now();
	...
	a.MouseTracker.wheelEventName =
		(a.Browser.vendor == a.BROWSERS.IE && a.Browser.version > 8) ||
	...
	if (
		...
		a.MouseTracker.subscribeEvents.push(
			'pointerover',
			'pointerout',
			'pointerdown',
			'pointerup',
		...
		'ontouchstart' in window &&
			a.MouseTracker.subscribeEvents.push('touchstart', 'touchend', 'touchmove', 'touchcancel');
		'ongesturestart' in window &&
			a.MouseTracker.subscribeEvents.push('gesturestart', 'gesturechange');
	...
	a.MouseTracker.GesturePointList.prototype = {
		...
		getById: function (a) {
			var b,
				c = this._gPoints.length;
			for (b = 0; c > b; b++) if (this._gPoints[b].id === a) return this._gPoints[b];
			return null;
		},
		getPrimary: function (a) {
			var b,
				c = this._gPoints.length;
			for (b = 0; c > b; b++) if (this._gPoints[b].isPrimary) return this._gPoints[b];
			return null;
		}
	};
	var qa = (function () {
...
!(function (a) {
	...
	a.Control = function (b, c, d) {
		...
		c.attachToViewer
			? this.anchor == a.ControlAnchor.TOP_RIGHT || this.anchor == a.ControlAnchor.BOTTOM_RIGHT
...
!(function (a) {
	function b(b) {
		b = a.getElement(b);
		return new a.Point(
			0 === b.clientWidth ? 1 : b.clientWidth,
		...
		function f(a, b) {
			if (a.ready) d(a);
				...
				a.addHandler('open-failed', function (a) {
					e({ message: a.message, source: b });
				});
			}
		}
		var g = b;
		'string' == a.type(c) &&
			(c.match(/\s*<.*/) ? (c = a.parseXml(c)) : c.match(/\s*[\{\[].*/) && (c = a.parseJSON(c)));
		setTimeout(function () {
			if ('string' == a.type(c)) {
				c = new a.TileSource({
					url: c,
				...
				!c.crossOriginPolicy && b.crossOriginPolicy && (c.crossOriginPolicy = b.crossOriginPolicy);
				...
				if (a.isFunction(c.getTileUrl)) {
					var h = new a.TileSource(c);
					h.getTileUrl = c.getTileUrl;
					d(h);
				} else {
					var i = a.TileSource.determineType(g, c);
					if (!i) {
						e({ message: 'Unable to load TileSource', source: c });
						return;
					}
					var j = i.prototype.configure.apply(g, [c]);
					f(new i(j), c);
				}
			} else f(c, c);
		});
		...
		switch (b.keyCode) {
			case 38:
				b.shift
					...
					: this.viewport.panBy(this.viewport.deltaPointsFromPixels(new a.Point(0, -40)));
	...
	function P() {
		this.isFullPage() && !a.isFullScreen()
			? this.setFullPage(!1)
	...
	a.Viewer = function (c) {
		...
		'undefined' != typeof U[this.hash] &&
			a.console.warn('Hash ' + this.hash + ' has already been used.');
		U[this.hash] = {
		...
		this.toolbar && (this.toolbar = new a.ControlDock({ element: this.toolbar }));
		this.bindStandardControls();
		U[this.hash].prevContainerSize = b(this.container);
		this.world = new a.World({ viewer: this });
		...
		this.world.addHandler('metrics-change', function (a) {
			g.viewport &&
				g.viewport._setContentBounds(g.world.getHomeBounds(), g.world.getContentFactor());
		});
		...
		this.imageLoader = new a.ImageLoader({ jobLimit: this.imageLoaderLimit });
		this.tileCache = new a.TileCache({ maxImageCacheCount: this.maxImageCacheCount });
		...
		this.showNavigator &&
			(this.navigator = new a.Navigator({
				id: this.navigatorId,
		...
		for (d = 0; d < this.customControls.length; d++)
			this.addControl(this.customControls[d].id, { anchor: this.customControls[d].anchor });
		a.requestAnimationFrame(function () {
			h(g);
		});
	};
	a.extend(a.Viewer.prototype, a.EventSource.prototype, a.ControlDock.prototype, {
				...
				if (this.sequenceMode && a.isArray(b)) {
					...
					if (b.length) {
						...
						var i = function () {
							if (f + g === e)
								if (f) {
									if (c._firstOpen || !c.preserveViewport) {
										c.viewport.goHome(!0);
										c.viewport.update();
									}
									c._firstOpen = !1;
									...
									if (c.overlays && !c.preserveOverlays)
										for (var i = 0; i < c.overlays.length; i++)
											c.currentOverlays[i] = d(c, c.overlays[i]);
									c._drawOverlays();
									c._opening = !1;
									c.raiseEvent('open', { source: a });
								} else {
									c._opening = !1;
									c.raiseEvent('open-failed', h);
								}
						};
						var j = function (b) {
							(a.isPlainObject(b) && b.tileSource) || (b = { tileSource: b });
							if (void 0 !== b.index) {
								a.console.error(
							...
							b.success = function (a) {
								...
								if (b.tileSource.overlays)
									for (var e = 0; e < b.tileSource.overlays.length; e++)
										c.addOverlay(b.tileSource.overlays[e]);
								d && d(a);
								i();
							};
							var e = b.error;
						...
						for (var k = 0; k < b.length; k++) j(b[k]);
						return this;
					}
				}
		},
		...
		setControlsEnabled: function (a) {
			a ? j(this) : h(this);
			this.raiseEvent('controls-enabled', { enabled: a });
			return this;
		},
		isFullPage: function () {
		...
		setFullPage: function (b) {
			...
			if (b) {
				...
				this.toolbar &&
					...
					(this.element.style.height =
						a.getElementSize(this.element).y - a.getElementSize(this.toolbar.element).y + 'px');
				...
				for (d = 0; c > d; d++) e.appendChild(this.previousBody.shift());
				...
				var k = function () {
					...
					j++;
					((10 > j && b.x !== h.pageScroll.x) || b.y !== h.pageScroll.y) &&
			...
			this.raiseEvent('full-page', { fullPage: b });
			return this;
		},
		...
		addTiledImage: function (b) {
			function d(a) {
				for (var c = 0; c < f._loadQueue.length; c++)
					if (f._loadQueue[c] === g) {
						f._loadQueue.splice(c, 1);
						break;
					}
				0 === f._loadQueue.length && e(g);
				f.raiseEvent('add-item-failed', a);
				b.error && b.error(a);
			}
			function e(a) {
				if (f.collectionMode) {
					f.world.arrange({
						...
						columns: f.collectionColumns,
						layout: f.collectionLayout,
						tileSize: f.collectionTileSize,
						tileMargin: f.collectionTileMargin
					});
					f.world.setAutoRefigureSizes(!0);
				}
			}
			...
			a.console.assert(
				!b.replace || (b.index > -1 && b.index < this.world.getItemCount()),
				'[Viewer.addTiledImage] if options.replace is used, options.index must be a valid index in Viewer.world'
			);
			var f = this;
			b.replace && (b.replaceItem = f.world.getItemAt(b.index));
			this._hideMessage();
			...
			if (a.isArray(b.tileSource))
				...
				c(
					...
					function (b) {
						g.tileSource = b;
						var c, d, h;
						for (; f._loadQueue.length; ) {
							c = f._loadQueue[0];
							if (!c.tileSource) break;
							f._loadQueue.splice(0, 1);
							if (c.options.replace) {
								var i = f.world.getIndexOfItem(c.options.replaceItem);
								-1 != i && (c.options.index = i);
								f.world.removeItem(c.options.replaceItem);
							}
							d = new a.TiledImage({
								viewer: f,
								source: c.tileSource,
								viewport: f.viewport,
								drawer: f.drawer,
								tileCache: f.tileCache,
								imageLoader: f.imageLoader,
								x: c.options.x,
								y: c.options.y,
								width: c.options.width,
								height: c.options.height,
								fitBounds: c.options.fitBounds,
								fitBoundsPlacement: c.options.fitBoundsPlacement,
								clip: c.options.clip,
								placeholderFillStyle: c.options.placeholderFillStyle,
								opacity: c.options.opacity,
								compositeOperation: c.options.compositeOperation,
								springStiffness: f.springStiffness,
								animationTime: f.animationTime,
								minZoomImageRatio: f.minZoomImageRatio,
								wrapHorizontal: f.wrapHorizontal,
								wrapVertical: f.wrapVertical,
								immediateRender: f.immediateRender,
								blendTime: f.blendTime,
								alwaysBlend: f.alwaysBlend,
								minPixelRatio: f.minPixelRatio,
								smoothTileEdgesMinZoom: f.smoothTileEdgesMinZoom,
								iOSDevice: f.iOSDevice,
								crossOriginPolicy: f.crossOriginPolicy,
								debugMode: f.debugMode
							});
							f.collectionMode && f.world.setAutoRefigureSizes(!1);
							f.world.addItem(d, { index: c.options.index });
							0 === f._loadQueue.length && e(c);
							1 !== f.world.getItemCount() || f.preserveViewport || f.viewport.goHome(!0);
							if (f.navigator) {
								h = a.extend({}, c.options, {
									replace: !1,
									originalTiledImage: d,
									tileSource: c.tileSource
								});
								f.navigator.addTiledImage(h);
							}
							c.options.success && c.options.success({ item: d });
						}
					},
					function (a) {
						a.options = b;
		...
		bindSequenceControls: function () {
			...
			if (this.showSequenceControl) {
				...
				if (g) {
					...
					this.toolbar
						? this.toolbar.addControl(this.pagingControl, { anchor: a.ControlAnchor.BOTTOM_RIGHT })
		...
		bindStandardControls: function () {
			...
			if (this.showNavigationControl) {
				...
				if (this.showZoomControl) {
					p.push(
						(this.zoomInButton = new a.Button({
					...
					p.push(
						(this.zoomOutButton = new a.Button({
				...
				this.showFullPageControl &&
					p.push(
						(this.fullPageButton = new a.Button({
				...
				if (this.showRotationControl) {
					p.push(
						(this.rotateLeftButton = new a.Button({
					...
					p.push(
						(this.rotateRightButton = new a.Button({
				...
				if (q) {
					...
					this.toolbar
						? this.toolbar.addControl(this.navControl, { anchor: a.ControlAnchor.TOP_LEFT })
		...
		addOverlay: function (b, c, f, g) {
			var h;
			h = a.isPlainObject(b) ? b : { element: b, location: c, placement: f, onDraw: g };
			...
			this.raiseEvent('add-overlay', { element: b, location: h.location, placement: h.placement });
			return this;
		},
		updateOverlay: function (b, c, d) {
			...
			if (f >= 0) {
				...
				U[this.hash].forceRedraw = !0;
				this.raiseEvent('update-overlay', { element: b, location: c, placement: d });
			}
			return this;
		},
		removeOverlay: function (b) {
			...
			if (c >= 0) {
				...
				U[this.hash].forceRedraw = !0;
				this.raiseEvent('remove-overlay', { element: b });
			}
			return this;
		},
...
!(function (a) {
	...
	a.Navigator = function (g) {
		...
		if (g.id) {
			...
			g.controlOptions = { anchor: a.ControlAnchor.NONE, attachToViewer: !1, autoFade: !1 };
		} else {
		...
		g = a.extend(!0, { sizeRatio: a.DEFAULT_SETTINGS.navigatorSizeRatio }, g, {
		...
		this.totalBorderWidths = new a.Point(2 * this.borderWidth, 2 * this.borderWidth).minus(
			this.fudge
		);
	...
	a.extend(a.Navigator.prototype, a.EventSource.prototype, a.Viewer.prototype, {
		...
		update: function (b) {
			...
			if (b && this.viewport) {
				...
				h = this.viewport
					.pixelFromPointNoRotate(f.getBottomRight(), !1)
		...
		addTiledImage: function (b) {
			...
			var e = a.extend({}, b, {
				success: function (a) {
					var b = a.item;
					b._originalForNavigator = d;
					c._matchBounds(b, d, !0);
					d.addHandler('bounds-change', function () {
						c._matchBounds(b, d);
					});
				}
			});
...
!(function (a) {
	...
	a.extend(a, {
		getString: function (c) {
			...
			f = h[e[d]];
			if ('string' != typeof f) {
				a.console.debug('Untranslated source string:', c);
				f = '';
			}
...
!(function (a) {
	function b(b) {
		...
		else d = e.match(/\s*[\{\[].*/) ? a.parseJSON(e) : e;
		return d;
	}
	a.TileSource = function (b, c, d, e, f, g) {
		...
		if (this.url) {
			...
			this.maxLevel =
				void 0 !== i.maxLevel && null !== i.maxLevel
					? i.maxLevel
					: i.width && i.height
					? Math.ceil(Math.log(Math.max(i.width, i.height)) / Math.log(2))
					: 0;
	...
	a.TileSource.prototype = {
		...
		getClosestLevel: function (b) {
			var c, d, e;
			for (c = this.minLevel; c < this.maxLevel; c++) {
				e = this.getNumTiles(c);
				d = new a.Point(
					Math.floor(b.x / this.getTileWidth(c)),
					Math.floor(b.y / this.getTileHeight(c))
				);
				if (e.x + 1 >= d.x && e.y + 1 >= d.y) break;
			}
			return Math.max(0, c - 1);
		},
		getTileAtPoint: function (b, c) {
			var d = c.times(this.dimensions.x).times(this.getLevelScale(b)),
				e = Math.floor(d.x / this.getTileWidth(b)),
				f = Math.floor(d.y / this.getTileHeight(b));
		...
		getImageInfo: function (c) {
			...
			e = function (b) {
				...
				if (d) {
					...
					f = new d(g);
					k.ready = !0;
					k.raiseEvent('ready', { tileSource: f });
				} else k.raiseEvent('open-failed', { message: 'Unable to load TileSource', source: c });
			};
			if (c.match(/\.js$/)) {
				d = c.split('/').pop().replace('.js', '');
				a.jsonp({ url: c, async: !1, callbackName: d, callback: e });
			} else
				a.makeAjaxRequest({
					url: c,
					...
					error: function (a, b) {
						...
						try {
							...
							var f;
							f = 'undefined' != typeof b && b.toString ? b.toString() : 'Unknown error';
...
!(function (a) {
	function b(b, d) {
		...
		if ('Image' == k)
			...
			if ('Error' == k) {
				var p = j.getElementsByTagName('Message')[0];
				var q = p.firstChild.nodeValue;
				throw new Error(q);
			}
		}
	...
	a.extend(a.DziTileSource.prototype, a.TileSource.prototype, {
		supports: function (a, b) {
			...
			a.Image
				? (c = a.Image.xmlns)
				: a.documentElement &&
				  ('Image' == a.documentElement.localName || 'Image' == a.documentElement.tagName) &&
				  (c = a.documentElement.namespaceURI);
			return (
				'http://schemas.microsoft.com/deepzoom/2008' == c ||
				'http://schemas.microsoft.com/deepzoom/2009' == c
			);
		},
		configure: function (d, e) {
			...
			if (e && !f.tilesUrl) {
				...
				-1 != e.search(/\.(dzi|xml|js)\?/)
					? (f.queryParams = e.match(/\?.*/))
					: (f.queryParams = '');
			}
			return f;
		},
...
!(function (a) {
	function b(a) {
		...
		var c = -1 != b.indexOf(a[0]);
		return !c || -1 != a.indexOf('sizeByW');
	}
	function c(a) {
		var b = [];
	...
	a.IIIFTileSource = function (d) {
		...
		d.maxLevel ||
			...
			(this.scale_factors
				? (d.maxLevel = Math.floor(Math.pow(Math.max.apply(null, this.scale_factors), 0.5)))
				: (d.maxLevel = Number(Math.ceil(Math.log(Math.max(this.width, this.height), 2)))));
	...
	a.extend(a.IIIFTileSource.prototype, a.TileSource.prototype, {
		supports: function (a, b) {
			return a.protocol && 'http://iiif.io/api/image' == a.protocol
				? !0
				: !a['@context'] ||
				  ('http://library.stanford.edu/iiif/image-api/1.1/context.json' != a['@context'] &&
						'http://iiif.io/api/image/1/context.json' != a['@context'])
				? a.profile &&
				  0 === a.profile.indexOf('http://library.stanford.edu/iiif/image-api/compliance.html')
					? !0
					: a.identifier && a.width && a.height
					? !0
					: a.documentElement &&
					  'info' == a.documentElement.tagName &&
					  'http://library.stanford.edu/iiif/image-api/ns/' == a.documentElement.namespaceURI
					? !0
					: !1
				: !0;
		},
		...
		getTileWidth: function (b) {
			...
			return this.tileSizePerScaleFactor && this.tileSizePerScaleFactor[c]
		...
		getLevelScale: function (b) {
			if (this.emulateLegacyImagePyramid) {
				...
				this.levels.length > 0 &&
					b >= this.minLevel &&
					b <= this.maxLevel &&
					(c = this.levels[b].width / this.levels[this.maxLevel].width);
				return c;
			}
		...
		getTileUrl: function (a, b, c) {
			...
			var e,
				...
				p,
				q = '0',
				r = Math.pow(0.5, this.maxLevel - a),
...
!(function (a) {
	function b(b) {
		...
		for (d = 0; d < b.length; d++) {
			...
			(c.url.toLowerCase().match(/^.*\.(png|jpg|jpeg|gif)(?:\?.*)?$/) ||
				(c.mimetype && c.mimetype.toLowerCase().match(/^.*\/(png|jpg|jpeg|gif)$/)))
				? e.push({ url: c.url, width: Number(c.width), height: Number(c.height) })
	...
	function c(b, c) {
		...
		if ('image' == h)
			try {
				i = { type: g.getAttribute('type'), levels: [] };
	...
	a.LegacyTileSource = function (c) {
		var d, e, f;
		a.isArray(c) && (d = { type: 'legacy-image-pyramid', levels: c });
		d.levels = b(d.levels);
	...
	a.extend(a.LegacyTileSource.prototype, a.TileSource.prototype, {
		supports: function (a, b) {
			return (
				...
				(a.documentElement && 'legacy-image-pyramid' == a.documentElement.getAttribute('type'))
			);
		},
	...
	a.extend(a.ImageTileSource.prototype, a.TileSource.prototype, {
		...
		getImageInfo: function (b) {
			...
			a.addEvent(c, 'load', function () {
				d.width = c.naturalWidth;
				d.height = c.naturalHeight;
				...
				d.ready = !0;
				d.raiseEvent('ready', { tileSource: d });
			});
		...
		_buildLevels: function () {
			var b = [
				{ url: this._image.src, width: this._image.naturalWidth, height: this._image.naturalHeight }
			];
			if (!this.buildPyramid || !a.supportsCanvas || !this.useCanvas) {
				delete this._image;
				return b;
			}
			var c = this._image.naturalWidth;
			var d = this._image.naturalHeight;
			...
			for (; c >= 2 && d >= 2; ) {
				...
				b.splice(0, 0, { context2D: h, width: c, height: d });
				e = g;
				f = h;
			}
			return b;
...
!(function (a) {
	...
	a.ButtonState = { REST: 0, GROUP: 1, HOVER: 2, DOWN: 3 };
	a.Button = function (b) {
		var c = this;
		a.EventSource.call(this);
		a.extend(
		...
		this.tracker = new a.MouseTracker({
			...
			enterHandler: function (b) {
				if (b.insideElementPressed) {
					...
					c.raiseEvent('enter', { originalEvent: b.originalEvent });
				} else b.buttonDownAny || f(c, a.ButtonState.HOVER);
			},
			focusHandler: function (a) {
				this.enterHandler(a);
				c.raiseEvent('focus', { originalEvent: a.originalEvent });
			},
			exitHandler: function (b) {
				g(c, a.ButtonState.GROUP);
				b.insideElementPressed && c.raiseEvent('exit', { originalEvent: b.originalEvent });
			},
			blurHandler: function (a) {
				this.exitHandler(a);
				c.raiseEvent('blur', { originalEvent: a.originalEvent });
			},
			pressHandler: function (b) {
				f(c, a.ButtonState.DOWN);
				c.raiseEvent('press', { originalEvent: b.originalEvent });
			},
			releaseHandler: function (b) {
				if (b.insideElementPressed && b.insideElementReleased) {
					g(c, a.ButtonState.HOVER);
					c.raiseEvent('release', { originalEvent: b.originalEvent });
			...
			keyHandler: function (a) {
				if (13 === a.keyCode) {
					c.raiseEvent('click', { originalEvent: a.originalEvent });
					c.raiseEvent('release', { originalEvent: a.originalEvent });
					return !1;
				}
				return !0;
			}
...
!(function (a) {
	a.Rect = function (a, b, c, d, e) {
		this.x = 'number' == typeof a ? a : 0;
		this.y = 'number' == typeof b ? b : 0;
		this.width = 'number' == typeof c ? c : 0;
		this.height = 'number' == typeof d ? d : 0;
		this.degrees = 'number' == typeof e ? e : 0;
		this.degrees = this.degrees % 360;
		this.degrees < 0 && (this.degrees += 360);
		var f, g;
		if (this.degrees >= 270) {
			f = this.getTopRight();
			this.x = f.x;
			...
			f = this.getBottomRight();
			this.x = f.x;
			this.y = f.y;
			this.degrees -= 180;
		} else if (this.degrees >= 90) {
			f = this.getBottomLeft();
			this.x = f.x;
			this.y = f.y;
			g = this.height;
			this.height = this.width;
	...
	a.Rect.prototype = {
		...
		getBottomRight: function () {
			return new a.Point(this.x + this.width, this.y + this.height).rotate(
				this.degrees,
				this.getTopLeft()
			);
		},
		...
		getCenter: function () {
			return new a.Point(this.x + this.width / 2, this.y + this.height / 2).rotate(
				this.degrees,
				this.getTopLeft()
			);
		},
...
!(function (a) {
	...
	function d(b, c, d) {
		...
		f = g - f;
		f = 0 > f ? 0 : f;
		for (j = f; g > j && j < b.panels.length; j++) {
			k = b.panels[j];
			if (!k.activePanel) {
				h = new a.Viewer({
					id: k.id,
	...
	function f(b) {
		...
		'horizontal' == this.scroll
			? (c.style.marginBottom = '-' + a.getElementSize(c).y / 2 + 'px')
			: (c.style.marginLeft = '-' + a.getElementSize(c).x / 2 + 'px');
		return !1;
	}
	function g(a) {
	...
	a.ReferenceStrip = function (j) {
		...
		if (j.width && j.height) {
			...
			o.addControl(this.element, { anchor: a.ControlAnchor.BOTTOM_LEFT });
		} else if ('horizontal' == j.scroll) {
			this.element.style.width =
				p.x * j.sizeRatio * o.tileSources.length + 12 * o.tileSources.length + 'px';
			this.element.style.height = p.y * j.sizeRatio + 'px';
			o.addControl(this.element, { anchor: a.ControlAnchor.BOTTOM_LEFT });
		} else {
			this.element.style.height =
				p.y * j.sizeRatio * o.tileSources.length + 12 * o.tileSources.length + 'px';
			this.element.style.width = p.x * j.sizeRatio + 'px';
			o.addControl(this.element, { anchor: a.ControlAnchor.TOP_LEFT });
		}
	...
	a.extend(a.ReferenceStrip.prototype, a.EventSource.prototype, a.Viewer.prototype, {
		setFocus: function (b) {
			...
			if (this.currentSelected !== f) {
				...
				this.currentPage = b;
				e.call(this, { eventSource: this.innerTracker });
			}
		},
		update: function () {
...
!(function (a) {
	...
	a.Spring = function (b) {
		...
		a.console.assert(
			'number' == typeof b.springStiffness && 0 !== b.springStiffness,
			'[OpenSeadragon.Spring] options.springStiffness must be a non-zero number'
		);
		a.console.assert(
			'number' == typeof b.animationTime && b.animationTime >= 0,
...
!(function (a) {
	function b(b) {
		a.extend(!0, this, { timeout: a.DEFAULT_SETTINGS.timeout, jobId: null }, b);
		this.image = null;
	}
	function c(a, b, c) {
		var d;
	...
	b.prototype = {
		...
		start: function () {
			...
			this.image.onabort = this.image.onerror = function () {
				a.errorMsg = 'Image load aborted';
				a.finish(!1);
			};
			this.jobId = window.setTimeout(function () {
				a.errorMsg = 'Image load exceeded timeout';
				a.finish(!1);
			}, this.timeout);
	...
	a.ImageLoader = function (b) {
		a.extend(
			!0,
			this,
			{ jobLimit: a.DEFAULT_SETTINGS.imageLoaderLimit, jobQueue: [], jobsInProgress: 0 },
			b
		);
	};
	a.ImageLoader.prototype = {
		addJob: function (a) {
			var d = this,
				...
				f = { src: a.src, crossOriginPolicy: a.crossOriginPolicy, callback: e, abort: a.abort },
				g = new b(f);
...
!(function (a) {
	...
	a.Tile.prototype = {
		...
		drawCanvas: function (b, c, d, e) {
			var f,
				g = this.position.times(a.pixelDensityRatio),
			...
			if (this.context2D || this.cacheImageRecord) {
				...
				if (this.loaded && f) {
					...
					c({ context: b, tile: this, rendered: f });
...
!(function (a) {
	...
	a.OverlayRotationMode = a.freezeObject({ NO_ROTATION: 1, EXACT: 2, BOUNDING_BOX: 3 });
	a.Overlay = function (b, c, d) {
		var e;
		e = a.isPlainObject(b) ? b : { element: b, location: c, placement: d };
	...
	a.Overlay.prototype = {
		_init: function (b) {
			...
			this.checkResize = void 0 === b.checkResize ? !0 : b.checkResize;
		...
		_getOverlayPositionAndSize: function (b) {
			...
			if (b.degrees && this.rotationMode !== a.OverlayRotationMode.NO_ROTATION)
				if (
					this.rotationMode === a.OverlayRotationMode.BOUNDING_BOX &&
					null !== this.width &&
					null !== this.height
				) {
		...
		_adjustBoundsForRotation: function (b, c) {
			...
			if (this.rotationMode === a.OverlayRotationMode.BOUNDING_BOX) {
				...
				return b.viewerElementToViewportRectangle(
					new a.Rect(d.position.x, d.position.y, d.size.x, d.size.y)
				);
			}
...
!(function (a) {
	a.Drawer = function (b) {
		...
		var c = arguments;
		a.isPlainObject(b) || (b = { source: c[0], viewport: c[1], element: c[2] });
		...
		b.source &&
			a.console.error('[Drawer] options.source is no longer accepted; use TiledImage instead');
	...
	a.Drawer.prototype = {
		...
		viewportToDrawerRectangle: function (b) {
			...
			return new a.Rect(
				c.x * a.pixelDensityRatio,
				c.y * a.pixelDensityRatio,
				...
				if (null === this.sketchCanvas) {
					...
					if (0 === this.viewport.getRotation()) {
						...
						this.viewer.addHandler('rotate', function e() {
							...
							d.sketchCanvas.width = a.x;
							d.sketchCanvas.height = a.y;
						});
					}
				}
		...
		blendSketch: function (b, c, d, e) {
			var f = b;
			a.isPlainObject(f) || (f = { opacity: b, scale: c, translate: d, compositeOperation: e });
			if (this.useCanvas && this.sketchCanvas) {
				...
				if (g)
					...
					this.context.drawImage(
						...
						h.x - i * c,
						h.y - j * c,
						(this.canvas.width + 2 * i) * c,
						(this.canvas.height + 2 * j) * c,
						-i,
						-j,
						this.canvas.width + 2 * i,
						this.canvas.height + 2 * j
					);
				}
		...
		drawDebugInfo: function (b, c, d) {
			if (this.useCanvas) {
				var e = this.context;
				e.save();
				e.lineWidth = 2 * a.pixelDensityRatio;
				e.font = 'small-caps bold ' + 13 * a.pixelDensityRatio + 'px arial';
				e.strokeStyle = this.debugGridColor;
				e.fillStyle = this.debugGridColor;
				0 !== this.viewport.degrees && this._offsetForRotation(this.viewport.degrees);
				e.strokeRect(
					b.position.x * a.pixelDensityRatio,
					b.position.y * a.pixelDensityRatio,
					b.size.x * a.pixelDensityRatio,
					b.size.y * a.pixelDensityRatio
				);
				var f = (b.position.x + b.size.x / 2) * a.pixelDensityRatio;
				var g = (b.position.y + b.size.y / 2) * a.pixelDensityRatio;
				e.translate(f, g);
				e.rotate((Math.PI / 180) * -this.viewport.degrees);
				e.translate(-f, -g);
				if (0 === b.x && 0 === b.y) {
					e.fillText(
						'Zoom: ' + this.viewport.getZoom(),
						b.position.x * a.pixelDensityRatio,
						(b.position.y - 30) * a.pixelDensityRatio
					);
					e.fillText(
				...
				e.fillText(
					'Row: ' + b.y,
					(b.position.x + 10) * a.pixelDensityRatio,
					(b.position.y + 40) * a.pixelDensityRatio
				);
				e.fillText(
					'Order: ' + d + ' of ' + c,
					(b.position.x + 10) * a.pixelDensityRatio,
					(b.position.y + 50) * a.pixelDensityRatio
				);
				e.fillText(
					'Size: ' + b.size.toString(),
					(b.position.x + 10) * a.pixelDensityRatio,
					(b.position.y + 60) * a.pixelDensityRatio
				);
				e.fillText(
		...
		debugRect: function (b) {
			if (this.useCanvas) {
				...
				c.strokeRect(
					b.x * a.pixelDensityRatio,
					b.y * a.pixelDensityRatio,
					b.width * a.pixelDensityRatio,
					b.height * a.pixelDensityRatio
				);
				c.restore();
			}
		},
		...
		_offsetForRotation: function (a, b) {
			var c = this.canvas.width / 2;
			var d = this.canvas.height / 2;
			var e = this._getContext(b);
			e.save();
			e.translate(c, d);
			e.rotate((Math.PI / 180) * a);
			e.translate(-c, -d);
		},
...
!(function (a) {
	a.Viewport = function (b) {
		...
		c.length &&
			...
			(b = { containerSize: c[0], contentSize: c[1], config: c[2] });
		if (b.config) {
			a.extend(!0, b, b.config);
			delete b.config;
		}
		this._margins = a.extend({ left: 0, top: 0, right: 0, bottom: 0 }, b.margins || {});
		delete b.margins;
		a.extend(
			!0,
			this,
	...
	a.Viewport.prototype = {
		...
		goHome: function (a) {
			this.viewer && this.viewer.raiseEvent('home', { immediately: a });
		...
		setMargins: function (b) {
			...
			this._margins = a.extend({ left: 0, top: 0, right: 0, bottom: 0 }, b);
			this._updateContainerInnerSize();
		...
		_applyBoundaryConstraints: function (b, c) {
			var d = new a.Rect(b.x, b.y, b.width, b.height);
			if (this.wrapHorizontal);
			else {
				var e = this.visibilityRatio * d.width;
				var f = d.x + d.width;
				var g = this._contentBoundsNoRotate.x + this._contentBoundsNoRotate.width;
				var h = this._contentBoundsNoRotate.x - f + e;
				var i = g - d.x - e;
				e > this._contentBoundsNoRotate.width
					? (d.x += (h + i) / 2)
					: 0 > i
					? (d.x += i)
					: h > 0 && (d.x += h);
			...
			if (this.wrapVertical);
				...
				var j = this.visibilityRatio * d.height;
				var k = d.y + d.height;
				var l = this._contentBoundsNoRotate.y + this._contentBoundsNoRotate.height;
				var m = this._contentBoundsNoRotate.y - k + j;
				var n = l - d.y - j;
				j > this._contentBoundsNoRotate.height
					? (d.y += (m + n) / 2)
					: 0 > n
					? (d.y += n)
					: m > 0 && (d.y += m);
			}
			this.viewer && this.viewer.raiseEvent('constrain', { immediately: c });
			return d;
		},
		applyConstraints: function (a) {
			...
			var e = this._applyBoundaryConstraints(d, a);
		...
		_fitBounds: function (b, c) {
			...
			if (e) {
				...
				h = this._applyBoundaryConstraints(h, d);
				g = h.getCenter();
			}
			if (d) {
				this.panTo(g, !0);
		...
		panTo: function (a, b) {
			...
			this.viewer && this.viewer.raiseEvent('pan', { center: a, immediately: b });
			return this;
		},
		...
		zoomTo: function (b, c, d) {
			...
			this.viewer && this.viewer.raiseEvent('zoom', { zoom: b, refPoint: c, immediately: d });
			return this;
		},
		setRotation: function (a) {
			...
			this.viewer.raiseEvent('rotate', { degrees: a });
			return this;
		},
		...
		resize: function (a, b) {
			...
			this.viewer && this.viewer.raiseEvent('resize', { newContainerSize: a, maintain: b });
		...
		_pixelFromPointNoRotate: function (b, c) {
			return b
				.minus(c.getTopLeft())
				.times(this._containerInnerSize.x / c.width)
				.plus(new a.Point(this._margins.left, this._margins.top));
		},
		...
		pointFromPixelNoRotate: function (b, c) {
			...
			return b
				.minus(new a.Point(this._margins.left, this._margins.top))
				.divide(this._containerInnerSize.x / d.width)
				.plus(d.getTopLeft());
		},
		...
		_viewportToImageDelta: function (b, c) {
			...
			return new a.Point(
				(b * this._contentSizeNoRotate.x) / d,
				(c * this._contentSizeNoRotate.x) / d
			);
		},
		viewportToImageCoordinates: function (b, c) {
			...
			return this._viewportToImageDelta(
				b - this._contentBoundsNoRotate.x,
				c - this._contentBoundsNoRotate.y
			);
		},
		...
		viewerElementToViewportRectangle: function (b) {
			return a.Rect.fromSummits(
				this.pointFromPixel(b.getTopLeft(), !0),
				this.pointFromPixel(b.getTopRight(), !0),
				this.pointFromPixel(b.getBottomLeft(), !0)
			);
		},
		viewportToViewerElementRectangle: function (b) {
			return a.Rect.fromSummits(
				this.pixelFromPoint(b.getTopLeft(), !0),
				this.pixelFromPoint(b.getTopRight(), !0),
				this.pixelFromPoint(b.getBottomLeft(), !0)
			);
		},
		...
		viewportToImageZoom: function (b) {
			if (this.viewer) {
				...
				if (c > 1)
					a.console.error('[Viewport.viewportToImageZoom] is not accurate with multi-image.');
				else if (1 === c) {
		...
		imageToViewportZoom: function (b) {
			if (this.viewer) {
				var c = this.viewer.world.getItemCount();
				if (c > 1)
					a.console.error('[Viewport.imageToViewportZoom] is not accurate with multi-image.');
				else if (1 === c) {
					var d = this.viewer.world.getItemAt(0);
...
!(function (a) {
	function b(b) {
		...
		var d,
			...
			o = !1,
			q = a.now(),
			r = b.viewport.getBoundsWithMargins(!0),
			s =
				b.viewport.deltaPixelsFromPointsNoRotate(b.source.getPixelRatio(0), !0).x *
				b._scaleSpring.current.value,
			t = Math.max(b.source.minLevel, Math.floor(Math.log(b.minZoomImageRatio) / Math.log(2))),
			u = Math.min(
				Math.abs(b.source.maxLevel),
				Math.abs(Math.floor(Math.log(s / b.minPixelRatio) / Math.log(2)))
			);
		for (; b.lastDrawn.length > 0; ) {
			d = b.lastDrawn.pop();
			d.beingDrawn = !1;
		}
		if (!b.wrapHorizontal && !b.wrapVertical) {
			var v = b.getClippedBounds(!0);
			var w = r.intersection(v);
			if (null === w) return;
			r = w;
		}
		r = r.getBoundingBox();
		r.x -= b._xSpring.current.value;
		r.y -= b._ySpring.current.value;
		var x = r.getTopLeft();
		var y = r.getBottomRight();
		if (
			(b.wrapHorizontal || !(y.x < 0 || x.x > b._worldWidthCurrent)) &&
			(b.wrapVertical || !(y.y < 0 || x.y > b._worldHeightCurrent))
		) {
			if (!b.wrapHorizontal) {
				x.x = Math.max(x.x, 0);
				y.x = Math.min(y.x, b._worldWidthCurrent);
			}
			if (!b.wrapVertical) {
				x.y = Math.max(x.y, 0);
				y.y = Math.min(y.y, b._worldHeightCurrent);
			}
			t = Math.min(t, u);
			var z;
			for (e = u; e >= t; e--) {
				z = !1;
				g =
					b.viewport.deltaPixelsFromPointsNoRotate(b.source.getPixelRatio(e), !0).x *
					b._scaleSpring.current.value;
				if ((!o && g >= b.minPixelRatio) || e == t) {
					z = !0;
					o = !0;
				} else if (!o) continue;
				h =
					b.viewport.deltaPixelsFromPointsNoRotate(b.source.getPixelRatio(e), !1).x *
					b._scaleSpring.current.value;
				i =
					b.viewport.deltaPixelsFromPointsNoRotate(
						b.source.getPixelRatio(
							Math.max(b.source.getClosestLevel(b.viewport.containerSize) - 1, 0)
						),
						!1
					).x * b._scaleSpring.current.value;
				j = b.immediateRender ? 1 : i;
				l = Math.min(1, (g - 0.5) / 0.5);
				m = j / Math.abs(j - h);
				n = c(b, o, z, e, l, m, x, y, q, n);
				if (k(b.coverage, e)) break;
			}
			p(b, b.lastDrawn);
			n && !n.context2D && f(b, n, q);
		}
	}
	function c(a, b, c, e, f, g, h, i, j, k) {
		...
		o = a.source.getTileAtPoint(e, h.divide(a._scaleSpring.current.value));
		p = a.source.getTileAtPoint(e, i.divide(a._scaleSpring.current.value));
		q = a.source.getNumTiles(e);
		n(a.coverage, e);
		a.wrapHorizontal || (p.x = Math.min(p.x, q.x - 1));
		a.wrapVertical || (p.y = Math.min(p.y, q.y - 1));
		for (l = o.x; l <= p.x; l++)
			for (m = o.y; m <= p.y; m++) k = d(a, c, b, l, m, e, f, g, r, q, j, k);
		return k;
	}
	function d(a, b, c, d, f, g, k, n, p, q, r, s) {
		var t = e(d, f, g, a.source, a.tilesMatrix, r, q, a._worldWidthCurrent, a._worldHeightCurrent),
			u = b;
		a.viewer && a.viewer.raiseEvent('update-tile', { tiledImage: a, tile: t });
		m(a.coverage, g, d, f, !1);
		if (!t.exists) return s;
		c && !u && (l(a.coverage, g, d, f) ? m(a.coverage, g, d, f, !0) : (u = !0));
		if (!u) return s;
		i(t, a.source.tileOverlap, a.viewport, p, n, a);
		if (!t.loaded)
			if (t.context2D) h(a, t);
			else {
				var v = a._tileCache.getImageRecord(t.url);
				if (v) {
					var w = v.getImage();
					h(a, t, w);
				}
		...
		if (t.loaded) {
			var x = j(a, t, d, f, g, k, r);
			x && (a._needsDraw = !0);
		} else t.loading || (s = o(s, t));
		return s;
	}
	function e(b, c, d, e, f, g, h, i, j) {
		...
		if (!f[d][b][c]) {
			k = (h.x + (b % h.x)) % h.x;
			l = (h.y + (c % h.y)) % h.y;
			m = e.getTileBounds(d, k, l);
			n = e.tileExists(d, k, l);
			o = e.getTileUrl(d, k, l);
			p = e.getContext2D ? e.getContext2D(d, k, l) : void 0;
			m.x += (b - k) / h.x;
			m.y += (j / i) * ((c - l) / h.y);
			f[d][b][c] = new a.Tile(d, b, c, m, n, o, p);
		}
		q = f[d][b][c];
		q.lastTouchTime = g;
		return q;
	...
	function g(b, c, d, e, f) {
		if (e)
			if (d < b.lastResetTime) {
				...
				var g = function () {
					var a = Math.ceil(Math.log(b.source.getTileWidth(c.level)) / Math.log(2));
					h(b, c, e, a);
				};
				b._midDraw ? window.setTimeout(g, 1) : g();
			}
		else {
			a.console.log('Tile %s failed to load: %s - error: %s', c, c.url, f);
			b.viewer.raiseEvent('tile-load-failed', { tile: c, tiledImage: b, time: d, message: f });
			c.loading = !1;
			c.exists = !1;
		}
	}
	function h(a, b, c, d) {
		...
		function f() {
			...
			if (0 === g) {
				b.loading = !1;
				b.loaded = !0;
				b.context2D || a._tileCache.cacheTile({ image: c, tile: b, cutoff: d, tiledImage: a });
				a._needsDraw = !0;
			}
		}
		var g = 0;
	...
	function p(b, c) {
		if (0 !== c.length) {
			...
			var e =
				b.opacity < 1 ||
				(b.compositeOperation && 'source-over' !== b.compositeOperation) ||
				(!b._isBottomItem() && d._hasTransparencyChannel());
			var f;
			var g;
			var h = b.viewport.getZoom(!0);
			var i = b.viewportToImageZoom(h);
			if (i > b.smoothTileEdgesMinZoom && !b.iOSDevice) {
				e = !0;
				f = d.getScaleForEdgeSmoothing();
				g = d.getTranslationForEdgeSmoothing(
					f,
					b._drawer.getCanvasSize(!1),
					b._drawer.getCanvasSize(!0)
				);
			}
			var j;
			if (e) {
				f ||
					(j = b.viewport
						.viewportToViewerElementRectangle(b.getClippedBounds(!0))
						.getIntegerBoundingBox()
						.times(a.pixelDensityRatio));
				b._drawer._clear(!0, j);
			}
			0 === b.viewport.degrees || f || b._drawer._offsetForRotation(b.viewport.degrees, e);
			var k = !1;
			if (b._clip) {
			...
			if (b.placeholderFillStyle && b._hasOpaqueTile === !1) {
				...
				var o = null;
				o =
					'function' == typeof b.placeholderFillStyle
			...
			for (var p = c.length - 1; p >= 0; p--) {
				...
				d.beingDrawn = !0;
				b.viewer && b.viewer.raiseEvent('tile-drawn', { tiledImage: b, tile: d });
			}
			k && b._drawer.restoreContext(e);
			0 === b.viewport.degrees || f || b._drawer._restoreRotationChanges(e);
			if (e) {
				var r = 0 !== b.viewport.degrees && f;
				r && b._drawer._offsetForRotation(b.viewport.degrees, !1);
				...
				r && b._drawer._restoreRotationChanges(!1);
			}
			q(b, c);
		}
	}
	function q(b, c) {
		if (b.debugMode)
			for (var d = c.length - 1; d >= 0; d--) {
				var e = c[d];
				try {
					b._drawer.drawDebugInfo(e, c.length, d);
				} catch (f) {
					a.console.error(f);
				}
			}
	...
	a.TiledImage = function (b) {
		...
		a.console.assert(
			!b.clip || b.clip instanceof a.Rect,
	...
	a.extend(a.TiledImage.prototype, a.EventSource.prototype, {
		...
		update: function () {
			var a = this._xSpring.current.value;
			var b = this._ySpring.current.value;
			var c = this._scaleSpring.current.value;
			this._xSpring.update();
			this._ySpring.update();
			this._scaleSpring.update();
			if (
				this._xSpring.current.value !== a ||
				this._ySpring.current.value !== b ||
				this._scaleSpring.current.value !== c
			) {
				this._updateForScale();
				this._needsDraw = !0;
				return !0;
		...
		getClippedBounds: function (b) {
			...
			if (this._clip) {
				var d = this._worldWidthCurrent / this.source.dimensions.x;
				var e = this._clip.times(d);
				c = new a.Rect(c.x + e.x, c.y + e.y, e.width, e.height);
			}
			return c;
		},
		...
		viewportToImageCoordinates: function (b, c, d) {
			...
			return d
				? this._viewportToImageDelta(
						b - this._xSpring.current.value,
						c - this._ySpring.current.value
				  )
				: this._viewportToImageDelta(
						b - this._xSpring.target.value,
						c - this._ySpring.target.value
				  );
		},
...
!(function (a) {
	...
	a.TileCache.prototype = {
		...
		getImageRecord: function (b) {
			a.console.assert(b, '[TileCache.getImageRecord] url is required');
		...
		_unloadTile: function (b) {
			...
			var e = this._imagesLoaded[c.url];
			e.removeTile(c);
			if (!e.getTileCount()) {
				e.destroy();
				delete this._imagesLoaded[c.url];
				this._imagesLoadedCount--;
			}
			d.viewer.raiseEvent('tile-unloaded', { tile: c, tiledImage: d });
		}
	};
})(OpenSeadragon);
!(function (a) {
	...
	a.extend(a.World.prototype, a.EventSource.prototype, {
		addItem: function (b, c) {
			...
			this.raiseEvent('add-item', { item: b });
		},
		...
		setItemIndex: function (b, c) {
			...
			if (c !== d && -1 !== d) {
				...
				this._needsDraw = !0;
				this.raiseEvent('item-index-change', { item: b, previousIndex: d, newIndex: c });
			}
		},
		...
		_figureSizes: function () {
			...
			(this._contentFactor === d && this._homeBounds.equals(b) && this._contentSize.equals(c)) ||
